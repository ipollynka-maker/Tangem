...
function layerScript(layer, index, spec) {
  ...
  // Enable 3D on ALL layers when any layer in the spec uses 3D —
  // ensures correct z-ordering and avoids Position arity mismatch in AE.
  const specHas3d = spec['3d'] || spec.layers.some(l =>
    l.property === 'translateZ' || l.property === 'rotateX' || l.property === 'rotateY'
  );

  // AE position: [x, y] for 2D layers, [x, y, z] for 3D layers
  function posValue(v) {
    if (layer.property === 'translateX') return specHas3d ? `[${v}, 0, 0]` : `[${v}, 0]`;
    if (layer.property === 'translateY') return specHas3d ? `[0, ${v}, 0]` : `[0, ${v}]`;
    if (layer.property === 'translateZ') return `[0, 0, ${v}]`;
    return specHas3d ? `[${v}, ${v}, 0]` : `[${v}, ${v}]`;
  }
  ...
  const enable3d = specHas3d ? `  layer${index}.threeDLayer = true;\n` : '';
...
function specToAeScript(spec) {
  ...
  camera.property("ADBE Transform Group").property("Position").setValue([comp.width/2 + ${camPos[0]}, comp.height/2 + ${camPos[1]}, ${camPos[2] || -500}]);
` : '';
